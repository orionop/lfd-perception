...
Remote collaboration with University of Twente / NAKAMA Robotics Lab. **The sole focus is vision-based recognition of objects relevant to robot tasks, extracted from task demonstration data, integrated into the lab's ROS 2 LfD pipeline.** The lab records bags on a Franka FR3 with a Bota wrist F/T sensor, a Franka gripper, and a ZED RGB camera. The lab owns data collection and export; this repo owns the downstream vision module.

**Archived, not in scope (kept for reference, not deleted):** camera-to-force-sensor calibration / wrench-projected geometric contact grounding (`archive/wrench_force_calibration/`) and the manuscript/paper workstream (`archive/manuscript_b_paper/`). Neither is required for A and the active pipeline has zero dependency on either — see `LAB_DELIVERABLE_A.md`'s Scope section for why they were archived.
...
- A is currently operational but semi-automatic, not complete. The open gates
  are held-out automatic object selection, an actual downstream ROS 2
  integration test, and reproducible handover.
...
- **Camera calibration / wrench-ray / CAD-extraction scripts are archived** (`project_ee.py`, `calibrate_hand_eye.py`, `wrench_ray.py`, `wrench_ray_validate.py`, `handeye_offset_search.py`, `verify_camera_intrinsics.py`, `verify_board_scale.py`, `generate_charuco_board.py`, `extrinsic_grid_search.py`, `cad_extract_transform.py`, `cad_find_lens_occ.py`, `cad_find_lens_planar.py`, `cad_isolate_body_solids.py`, `cad_candidate_sensitivity.py`, `sim_wrench_ray_validation.py`, `mark_sheet_decode.py`, `mark_sheet_azimuth.py`, `pose_frame_probe.py`, `audit_extrinsic.py`, `why_extrinsic.py`, `verify_mark_transform.py`, `derive_tool_axis.py`, `measure_grasp_offset.py`, `grasp_offset_search.py`, `contact_seed_pixel.py`, `contact_eval_set.py`, `geometric_seed.py`, `grasped_weight_ray.py`, `calibration_sensitivity_sweep.py`) — moved to `archive/wrench_force_calibration/` 2026-09-10. Not required for Deliverable A's scope (vision-based object recognition); see that directory's `README.md` and `CALIBRATION_LOG.md` for the full six-hypotheses-rejected history. The active pipeline has zero dependency on any of it.
...
- **Research bar** (applies only if the archived manuscript workstream is ever resumed, see `archive/manuscript_b_paper/`): any paper framing of this work must be genuinely novel, not "we composed N existing tools." Systems-integration framings have been explicitly rejected.
- **Simulation is not a substitute for real-hardware validation** when a claim is specifically about overcoming a real-world measurement/calibration difficulty (see `archive/wrench_force_calibration/sim_wrench_ray_validation.py`'s docstring for why). A simulation that defines the exact quantity being measured doesn't validate the claim — it sidesteps it. Fine as a cheap pre-flight sanity check, never as evidence. General principle, kept even though the calibration work it originally applied to is archived.
