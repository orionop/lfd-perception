...
- The Franka Research 3 arm URDF is vendored at `Data/fr3.urdf` (arm-only, flange `fr3_link8`); the stock Franka Hand TCP is `+0.1034 m` z past the flange, but this rig's TCP was reconfigured. `current_pose` is published in the base frame. **Which frame it reports is RESOLVED (2026-09-10, closing the question open since 2026-08-28):** Vlutters confirmed via a Franka Desk End Effector panel screenshot (`Data/`-external, "Bota + Hand + Bracket_r2 + ZED" profile) that Flange→TCP is configured as `(0, 0, 0.035)` m with zero rotation, and "the TCP is placed 35 mm lower than the flange, such that it coincides with the measurement frame of the force sensor" — `current_pose` (`franka_robot_state_broadcaster`'s `O_T_EE`) therefore already reports the Bota SensONE's own F/T measurement frame directly, zero further offset needed. `calibration.yaml`'s `end_effector` block now reflects this (`current_pose_is: bota_origin`, `bota_to_tcp: [0,0,0]`) instead of the prior `null`. This also retroactively explains `Code/handeye_offset_search.py`'s 2026-09-10 result (no bounded offset search improved the hand-eye candidate's 3/7): there was never a frame-origin bug for an offset to fix, so the 3/7-vs-6/7 gap has a different cause — most likely the capture's continuous motion corrupting the recovered rotation (see `calibrate_hand_eye.py` above), not calibrated away by any offset. **`Code/pose_frame_probe.py` could never have settled this** — its `score()` does `T_bb @ p` then projects with `T_bb @ T_bc`, so `current_pose` cancels algebraically and every sample yields the same pixel; its "zero offset 8/17 vs all six 104 mm placements 0/17" was evidence about `T_bota_camera` and the offset jointly, NOT about which frame the topic publishes.
...
- `calibrate_hand_eye.py` — recovers `bota_to_camera` by direct hand-eye calibration (OpenCV `cv2.calibrateHandEye`, AX=XB, eye-in-hand) instead of reading it off the CAD drawings. Supersedes `cad_extract_transform.py`/`cad_find_lens_occ.py` — the CAD reading left the lens position ambiguous between two 63mm-apart candidates, and `cad_candidate_sensitivity.py` showed both candidates project the wrench ray entirely outside the image on every trial tested (rotation is unreliable, not just the translation). Needs a physically-measured ChArUco board and a short recording of the arm pausing at several distinct poses in front of it (board fixed in the world, not held by the gripper). Both the ArUco/ChArUco detection stage and the AX=XB solve were validated against synthetic data before ever touching real captures (0.0° / ~1e-16m recovery error on a synthetic ground-truth transform) — the math is known-correct; real rig time only needs to validate detection quality. Prints the result for manual review; never writes `calibration.yaml` automatically. **First real run (2026-09-09, `lfdws_t002` calibration recording Mark sent, our own sent ChArUco board): 81,853/93,888 rows detected, 162 independent poses after dedup, board-position residual std-dev 3-5mm — tight and self-consistent — but scored 3/7 on `wrench_ray_validate.py`'s real-event test, worse than the untrusted CAD candidate's 6/7. Rejected; not pasted into `calibration.yaml`.** Tight internal residual proves the solver and rig motion are self-consistent, not that the transform is metrically correct: AX=XB can't detect a constant offset between `current_pose`'s reported origin and the true Bota measurement origin, which would explain the gap and ties directly to the still-open `current_pose` frame question. **Both the frame-offset hypothesis (`handeye_offset_search.py`) and this motion hypothesis are now rejected (2026-09-10)**: the frame question turned out to have zero offset by design (Vlutters confirmed `current_pose` already is the Bota origin — see the pose-frame note below), and `--max_speed_mps 0.001` re-run on the same recording (17 independent poses from genuinely-static instants, residual std-dev 3.2/5.6/1.4mm) reproduces essentially the same rotation and the identical 3/7 wrench-ray score, same events hit and missed. Both scripts were re-audited during this test (index alignment against `df.iterrows()`, `quat_to_R` diffed byte-for-byte against `wrench_ray.py`'s copy) — no bug found in either. The 3/7-vs-6/7 gap remains unexplained; see `LAB_DELIVERABLE_A.md` for the two live possibilities (a real still-unfound calibration error, or the 7-event test itself being too thin to trust either candidate's score). Do not re-run this calibration on the same recording expecting a different number.
...
- `handeye_offset_search.py` — tests whether the 2026-09-09 hand-eye calibration's 3/7 (vs. the adopted candidate's 6/7) is explained by a fixed translational offset between `current_pose`'s reported origin and the Bota SensONE's true F/T origin (the still-open frame question), by holding the hand-eye rotation fixed and grid-searching a bounded (±50mm, motivated by the sensor's 38mm datasheet thickness) translation correction against the same 7-event wrench-ray harness. **Rejected, same LORO-CV + empirical-null discipline as `extrinsic_grid_search.py`:** the best in-fold candidate sits at the search boundary, 966/9,261 candidates (10.4% of the grid) tie at the same best rate spanning the full ±50mm range on every axis (a plateau, not a peak), and every leave-one-recording-out fold scores 0/1 or 0/5 — zero generalization. A simple constant origin offset does not explain the gap; the rotation itself is the more likely culprit, which points back at the motion/sync issue (see `calibrate_hand_eye.py` below) rather than the frame question per se.
